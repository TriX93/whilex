 ...
 */
/**
* <h1>Class Database extends PDO (Native API from PHP 5+)</h1>
* <p>This class is a wrapper/Pseudo-ORM for the Database. It allows you to create a connection, do some magic tricks with OOP and PDO (see test file for detail)
* Pointed as final because I don't think that there'll be any extension of that.</p>
* @author Claudio Ludovico Panetta (@ludo237)
* @version 0.1 _TESTED (test file for reference)
* @example This Class it's easy to use, just create a new object $database = Database::init(); (NOT THE CONSTRUCT) then you can make query like in the test files
* <span>ChangeLog</span>
* <ul>
* <li> V: 0.1 - Add All this shit LoL</li>
* </ul>
*/
final class Database extends PDO {
    /*** Constant (private static object for Java guru) ***/
    const DB_NAME = "whilex"; // Constant configuration for Database Name
    const DB_HOST = "localhost"; // Constant configuration for Database host 
    const DB_USER = "root"; // Constant configuration for Database user
    const DB_PASSWORD = "";  // Constant configuration for Database password
    /*** Attribute ***/
    private $error;
    private $sql;
    private $bind = array();
    ...
    /**
    * HACK THE CONSTRUCT 
    * function init allow us to handle the exception because a "good programming path" doesn't allow to generate return in 
    * the __construct() method.
    * @return Database $databaseHandler <p>The object of the database</p>
    **/
    public static function init(){
        $databaseHandler = null;
        try{
            $databaseHandler = new self();
     ...
     * <p>The main method of the class, it should be private but for now it remains public (I need to fix this shit)</p>
     * <span>TODO:</span>
     * <ul> 
     * <li>Make this method private</li>
     * </ul>
     */
    public function __construct(){
        ...
        parent::__construct("mysql:host=".self::DB_HOST.";dbname=".self::DB_NAME, self::DB_USER, self::DB_PASSWORD,$options);
        $this->setAttribute(PDO::ATTR_STATEMENT_CLASS, array('DatabaseStatement'));
    }
    /**
     ...
     * prepare-time.
     * </p>
     * @return DatabaseStatement If the database server successfully prepares the statement unless otherwise specified in the $driver_options argument,
     * db::prepare returns a
     * DatabaseStatement object.
    ...
    public function prepare($sql, $driver_options = array()) {
        if(!isset($driver_options[PDO::ATTR_STATEMENT_CLASS])){
            $driver_options[PDO::ATTR_STATEMENT_CLASS] = array('DatabaseStatement');
        }
     ...
     * Execute the query.
     * @return mixed
     * Returns false on failure, returns a DatabaseStatement on succes.
     */
    public function run() {
        if(empty($this->sql)){
            $this->build();
